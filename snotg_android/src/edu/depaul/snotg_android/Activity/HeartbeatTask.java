...
import static edu.depaul.snotg_android.SnotgAndroidConstants.HEARTBEAT_INTERVAL;
import static edu.depaul.snotg_android.SnotgAndroidConstants.STATE_USERID;
import static edu.depaul.snotg_android.SnotgAndroidConstants.URL_PATH_HEARTBEAT;
...
public class HeartbeatTask extends AsyncTask<URL, Integer, Long> {
	...
	protected Long doInBackground(URL... params) {
		...
		while (true) {
			...
			HeartbeatUpdateJson json = new HeartbeatUpdateJson(lat, lon,
					STATE_USERID);
			jsonUserLocation = JsonUtility.sendRequest(URL_PATH_HEARTBEAT,
					getQueryParams(json));
			Log.i("Heartbeat", new Date().toString() + ", Latitude: " + lat
					+ ", Longitude: " + lon + " for:" + STATE_USERID
					+ " --- Returned:  " + jsonUserLocation);
